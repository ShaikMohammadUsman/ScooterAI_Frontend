...
export const LoadingDots: React.FC<{ bg: string, Icon?: IconType, logo?: StaticImageData }> = ({ bg, Icon, logo }) => {
    return (
        <div className="flex ml-2">
            {
                (logo || Icon) && <div className="flex justify-center items-center relative h-16 w-16 rounded-full bg-white">
                    {
            ...
            <div className={`flex items-center space-x-1.5 ml-5 text-black font-sans text-left p-6 text-[16px] rounded-xl w-fit bg-[${bg}]`}>
                ...
                <div className=" w-2 h-2 rounded-full bg-slate-700 animate-bounce delay-100" />
                <div className="w-2 h-2 rounded-full bg-slate-700 animate-bounce delay-200" />
            </div>
        </div>
    );
};