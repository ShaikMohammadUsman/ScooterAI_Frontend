...
export default function InterviewScoreCard({ candidate }: InterviewScoreCardProps) {
    ...
    const getVideoScores = () => {
        ...
        return {
            ...
            overall: normalizeToFive(evaluation?.overall_score)
        };
    };

    // Normalize scores to 0–5 scale
    const normalizeToFive = (score?: number | null) => {
        if (score === undefined || score === null) return 0;
        return score > 5 ? score / 20 : score;
    };

    // Get audio interview scores (normalized to /5)
    const getAudioScores = () => {
        ...
        return {
            averageScore: normalizeToFive(summary.average_score),
            credibilityScore: normalizeToFive(summary.credibility_score),
            communicationScore: normalizeToFive(summary.communication_score),
    ...
    const getOverallScore = () => {
        ...
        if (videoScores) {
            totalScore += videoScores.overall * 0.6; // /5 scale
            totalWeight += 0.6;
        }

        if (audioScores) {
            totalScore += audioScores.averageScore * 0.4; // /5 scale
            totalWeight += 0.4;
        }
                    ...
                    <div className="text-center p-6 bg-gradient-to-r from-blue-50 to-purple-50 rounded-xl border border-blue-200">
                        ...
                        <div className="relative inline-flex items-center justify-center">
                            <div className="w-24 h-24 rounded-full border-4 border-gray-200 flex items-center justify-center">
                                <span className={`text-2xl font-bold ${getScoreColor(overallScore, 5)}`}>
                            ...
                            <div className="absolute inset-0 w-24 h-24 rounded-full border-4 border-transparent border-t-blue-500 border-r-blue-500"
                                style={{
                                    transform: `rotate(${(overallScore / 5) * 360}deg)`,
                                    transition: 'transform 1s ease-in-out'
                                }}>
                            </div>
                        </div>
                        <p className="text-sm text-gray-600 mt-2">Combined Score (out of 5)</p>
                    </div>
                )}
                    ...
                    {audioScores && (
                        <Card className="border border-green-200 bg-green-50/30">
                            ...
                            <CardContent className="space-y-4">
                                <div className="space-y-3">
                                    {[
                                        { label: 'Average Score', score: audioScores.averageScore, max: 5 },
                                        { label: 'Credibility', score: audioScores.credibilityScore, max: 5 },
                                        { label: 'Communication', score: audioScores.communicationScore, max: 5 }
                                    ].map((item, index) => (
                                        <div key={index} className="space-y-2">
                                            <div className="flex justify-between items-center">
                                                <span className="text-sm font-medium text-gray-700">{item.label}</span>
                        ...
                        {audioScores && (
                            <div className="text-center p-3 bg-white rounded-lg border">
                                <div className="text-2xl font-bold text-green-600">
                                    {audioScores.averageScore.toFixed(1)}/5
                                </div>
                                <div className="text-sm text-gray-600">Audio Score</div>
                                <div className="text-xs text-gray-500 mt-1">
                                    {audioScores.averageScore >= 4 ? 'Excellent' :
                                        audioScores.averageScore >= 3 ? 'Good' : 'Needs Improvement'}
                                </div>
                            </div>
                        )}
                        {overallScore && (
                            <div className="text-center p-3 bg-white rounded-lg border">
                                ...
                                <div className="text-xs text-gray-500 mt-1">
                                    {overallScore >= 4 ? 'Excellent' :
                                        overallScore >= 3 ? 'Good' : 'Needs Improvement'}
                                </div>
                            </div>
                        )}
                    </div>
